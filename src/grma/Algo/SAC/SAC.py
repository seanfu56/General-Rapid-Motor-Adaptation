...
class ReplayBuffer:
    ...
    def __init__(self, obs_dim, act_dim, size):
        self.obs_buf = np.zeros(Model.combined_shape(size, obs_dim), dtype=np.float32)
        self.obs2_buf = np.zeros(Model.combined_shape(size, obs_dim), dtype=np.float32)
        self.act_buf = np.zeros(Model.combined_shape(size, act_dim), dtype=np.float32)
...
def sac(env_fn, actor_critic=Model.MLPActorCritic, ac_kwargs=dict(), seed=0, 
    ...
    obs_dim = env.observation_space.shape[0] + act_dim
    ...
    def test_agent():
        ...
        for j in range(num_test_episodes):
            ...
            while not(d or (ep_len == max_ep_len)):
                ...
                o, r, d, _, _ = test_env.step(get_action(np.append(o, np.zeros(act_dim)), True))
                ep_ret += r
                ep_len += 1
    ...
    noise = np.random.normal(0, std, act_dim)
    ...
    for t in range(total_steps):
        ...
        if t > start_steps:
            a = get_action(np.append(o, noise))
        else:
            a = env.action_space.sample()

        # Step the env

        real_a = a * (np.ones(act_dim) + noise)

        o2, r, d, _, _ = env.step(real_a)
        ep_ret += r
        ep_len += 1
        ...
        replay_buffer.store(np.append(o, noise), a, r, np.append(o2, noise), d)
        ...
        if d or (ep_len == max_ep_len):
            ...
            if(epoch < noise_warm_epochs):
                std = noise_std_min + (noise_std_max - noise_std_min) * epoch / noise_warm_epochs
            else:
                std = noise_std_max
        ...
        if (t+1) % steps_per_epoch == 0:
            ...
            if(epoch % 10 == 0):            
                ...
                print(f"Epoch: {epoch}, Timestep: {t+1}, Reward: {test_reward:.2f}, Noise_std: {std:.4f}, Time: {s//3600:02}:{s%3600//60:02}:{s%60:02}", )

                with open(f'{save_dir}/training.log', 'a') as file:
                    file.write(f'Epoch: {epoch}, Timestep: {t+1}, Reward: {test_reward:.2f}, Noise_std: {std:.4f}, Time: {s//3600:02}:{s%3600//60:02}:{s%60:02}\n')
